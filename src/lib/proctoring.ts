...
export interface ViolationEvent {
  id: string;
  type: 'tab_switch' | 'window_blur' | 'fullscreen_exit' | 'right_click' | 'dev_tools' | 'network_disconnect' | 'camera_disabled' | 'microphone_disabled';
  timestamp: Date;
...
export interface MonitoringProcess {
  id: string;
  type: 'webcam' | 'screen_share' | 'keyboard' | 'mouse' | 'network' | 'browser_lock';
...
const saveSessionData = async (session: ProctoringSession, summary: SessionSummary): Promise<void> => {
  try {
    // Save to Firestore
    ...
    // Local backup
    localStorage.setItem(`proctoring_backup_${session.sessionId}`, JSON.stringify(backupData));
...
const generateSessionSummary = (session: ProctoringSession): SessionSummary => {
  ...
  return {
    ...
    dataIntegrity: {
      recordsCollected: session.violations.length + session.monitoringProcesses.length,
...
const calculateOverallPerformanceScore = (session: ProctoringSession): number => {
...
const calculateFullScreenCompliance = (session: ProctoringSession): number => {
  const fullScreenViolations = session.violations.filter(v => v.type === 'fullscreen_exit').length;
  const duration = session.endTime ? session.endTime.getTime() - session.startTime.getTime() : 0;
...
const calculateCameraActiveTime = (session: ProctoringSession): number => {
  const cameraProcess = session.monitoringProcesses.find(p => p.type === 'webcam');
...
// Recovery mechanisms
const handleRecoveryScenarios = async (sessionId: string, errorType: string): Promise<string[]> => {
...
export const endProctoring = async (
  ...
  try {
    ...
    try {
      ...
      try {
        localStorage.setItem(`emergency_backup_${sessionId}`, JSON.stringify({
          session,
          summary,
    ...
    if (errors.length > 0) {
      ...
      recoveryActions = await handleRecoveryScenarios(sessionId, 'incomplete_termination');
    }
    ...
    if (typeof window !== 'undefined') {
      ...
      if (summary.violationCount > 0) {
        toast.info(`Session completed with ${summary.violationCount} violations recorded`, {
          duration: 7000
        });
      }
    }
    ...
    return {
      ...
      metadata: {
        ...
        recoveryActions: await handleRecoveryScenarios(sessionId, 'critical_error')
      }
    };
  }
};
...
export const addViolation = (sessionId: string, violation: Omit<ViolationEvent, 'id'>): void => {
  const session = activeSessions.get(sessionId);
  if (session) {
...
export const addMediaStream = (sessionId: string, stream: MediaStream): void => {
  const session = activeSessions.get(sessionId);
  if (session) {
...
export const addMonitoringProcess = (sessionId: string, process: MonitoringProcess): void => {
  const session = activeSessions.get(sessionId);
  if (session) {
...
export const registerTerminationCallback = (sessionId: string, callback: () => void): void => {
  const callbacks = terminationCallbacks.get(sessionId) || [];
...
export const checkBrowserCompatibility = (): {
  ...
  if (!features.mediaDevices) warnings.push('Camera/microphone access not supported');
  ...
  if (!features.webRTC) warnings.push('WebRTC not supported - limited monitoring capabilities');
  
  const compatible = features.mediaDevices && features.fullscreen && features.localStorage;
