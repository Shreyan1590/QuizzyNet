...
const StudentDashboard: React.FC = () => {
  ...
  const initializeRealTimeListeners = () => {
    ...
    // Real-time listener for quizzes - removed orderBy to avoid composite index requirement
    ...
    // Real-time listener for quiz results - removed orderBy to avoid composite index requirement
    ...
    const unsubscribeResults = onSnapshot(resultsQuery, (snapshot) => {
      ...
      // Sort results by completedAt in client code instead of using orderBy in query
      ...
      calculatePerformanceStats(sortedResults);
    });
  ...
  const processQuizData = (quizzes: Quiz[]) => {
    ...
    // Sort quizzes by scheduledAt in client code
    const sortedQuizzes = relevantQuizzes.sort((a, b) => {
    ...
    const upcoming = sortedQuizzes.filter(quiz => {
